...
def _infer_hops(record: Dict[str, Any], default: int) -> int:
    ...
    if isinstance(facts, list) and facts:
        ...
        if titles: return max(1, len(titles))
...
def normalize_record(record: Dict[str, Any], index: int, default_hop: int) -> Sample:
    return Sample(
        ...
        answer=str(_first(record, ["answer", "gold_answer", "target", "output"])),
        hop_num=_infer_hops(record, default_hop),
        supporting_facts=_first(record, ["supporting_facts", "supporting facts", "supportingFacts", "evidence"], []),
        raw=record)
...
def load_dataset(path: str, dataset: str, default_hop: int = 2) -> List[Sample]:
    ...
    if name not in supported: raise ValueError(f"Unsupported dataset: {dataset}")
